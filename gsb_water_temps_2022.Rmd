...
Both temperature loggers successfully collected data from the time they were deployed, May 10, 2022 through October 21, 2022. Only data from one logger is represented on this page but data from both loggers is available as a .csv [here]('https://github.com/afstarke/waterTemps/tree/7163fa4d1c2c9cd64598fb3d357a825fbb710c56/datashare')
...
adjusted_cdf_plotly <- plot_ly(
  ...
  add_trace(y = ~F_unitless,
            ...
            hovertemplate = paste(
                        '<b>%{x:.1%}</b> measures made \nwere above %{y:.1f}\u00B0F <extra> This is equal to a total of <b>%{text:.0f}</b> days </extra>'),
            sizes = I(0.1)) %>% 
  layout(xaxis = list(title = "Proportions of measures"),
         yaxis = list(title = "\u00B0F", side = 'right'),
         title = 'During the monitoring period..')
...
subplot(line_plotly, adjusted_cdf_plotly, widths = c(0.75, 0.25), heights = c(0.5), margin = 0.04, shareY = T) %>%  
  layout(title = "Water temperatures through the summer of 2023", 
         showlegend = F,
         xaxis = list(),
         yaxis = list(title = "\u00B0F")) %>% 
...
Data linked here: [Temperature Data](https://github.com/afstarke/waterTemps/tree/7163fa4d1c2c9cd64598fb3d357a825fbb710c56/datashare)
