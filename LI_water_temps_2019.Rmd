...
stationMap <- mapview(estuarySiteLocations, popup = popupLink(siteName = estuarySiteLocations$station_nm, siteNo = estuarySiteLocations$site_no))
...
## Anomolies?  
How do the temps of 2019 compare with the past? Any 
Red ribbon = 2019 temperature range based on daily max and min at each station. Grey ribbon is daily average max and min by day for the previous 6 years. 
...
anomPlot <- daily_mean_6yr %>% ggplot(aes(x = commonDate, y = temp_mean7yr)) + 
  ...
  # scale_x_date(labels = function(x) format(x, "%d-%b")) +
  theme(legend.position = "none", axis.text.x = element_text(angle = 90, hjust = 1)) + 
  facet_grid(station_nm ~ .) + theme(panel.spacing = unit(1, "lines")) +
...
oxygen  %>% 
  ggplot(aes(x = commonDate, y = DO, color = DO, group = year)) +
  geom_line() + 
  # scale_color_gradient(low = "blue", high = "red", aesthetics = "color") + 
  scale_color_viridis_c(option = "inferno") +
  facet_grid(station_nm ~ year) + 
